...
짝을 이루는, **저장소를 처음 public으로 전환하기 직전** 실행할 점검 목록.

- [ ] **시크릿 스캔**: 커밋 히스토리 전체에 대해 [gitleaks](https://github.com/gitleaks/gitleaks)
      또는 GitHub의 push protection / secret scanning으로 전수 스캔. 발견 시
      `git filter-repo`로 히스토리에서 제거 후 해당 시크릿은 즉시 폐기(rotate).
- [ ] **커밋 히스토리 점검**: 개인 식별 정보(예: 실제 고객 데이터, 내부 전용 메모,
      테스트용으로 넣었던 실제 API 키)가 diff에 남아있지 않은지 확인.
...
- **외부 기여는 코어(AGPL 영역)만 받는다.** `ee/` 하위 변경(기능 추가, 리팩터링 포함)은
  ...
  - 이유: `ee/`는 상용 라이선스 코드이자 수익 모델의 핵심(`docs/01_BUSINESS_MODEL.md`)이라
...
- 라이선스 결정의 배경(왜 AGPL+ee인가)은 `docs/01_BUSINESS_MODEL.md` §5가 출처다.
